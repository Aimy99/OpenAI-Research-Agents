...
# Importing Agents and Responses
from working_agents import QueryResponse, SearchResult, FollowUpDecisionResponse
from working_agents import query_agent, search_agent,  synthesis_agent, follow_up_decision_agent

# Styling imports frm Rich
...
# Generating user query
    async def generate_queries(self) -> QueryResponse:
        with console.status("[bold cyan]Analyzing query...[/bold cyan]") as status:
            ...
            console.print(
                f"[yellow]Thoughts:[/yellow] {result.final_output.thoughts}")
...
# Searching the Web
    def duckduckgo_search(self, query: str):
        try:
            results = DDGS().text(query, region='us-en',
                                  safesearch='on', timelimit='y', max_results=5)
            return results
